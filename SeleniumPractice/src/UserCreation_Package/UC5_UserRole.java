...
public class UC5_UserRole extends UC2_Login {

	public void validateUserRole() throws Exception
	{
		WebElement f = driver.findElement(By.id("roleCode"));
		Select s = new Select(f); List<WebElement> opts = s.getOptions(); String fn = "User Role";
		...
		for (int i = 0; i < opts.size(); i++) {
			s.selectByIndex(i); Thread.sleep(300);
			logInfo(fn, "Select index " + i, s.getFirstSelectedOption().getText());
		}

		s.selectByIndex(1); String finalVal = s.getFirstSelectedOption().getText();
		log(fn, "Final value set (index 1)", "Non-empty", finalVal, !finalVal.isEmpty());
	}
}
