...
public class FU6_Result extends FU2_Login {

	@Test
	public void validateResult() throws Exception
	{
		System.out.println("========== RESULT (Dropdown) ==========");
		int caseNo = 1;

		WebElement dd = driver.findElement(By.id("callStatus"));
		Select sel = new Select(dd);

		boolean disp = dd.isDisplayed();
		System.out.println("Case " + caseNo + " | displayed | Expected: true | Actual: " + disp);
		sa.assertTrue(disp, "BUG: callStatus not displayed | Expected: true | Actual: " + disp);
		caseNo++;

		boolean en = dd.isEnabled();
		System.out.println("Case " + caseNo + " | enabled | Expected: true | Actual: " + en);
		sa.assertTrue(en, "BUG: callStatus not enabled | Expected: true | Actual: " + en);
		caseNo++;

		String tag = dd.getTagName();
		System.out.println("Case " + caseNo + " | tagName | Expected: select | Actual: " + tag);
		sa.assertEquals(tag, "select", "BUG: callStatus tagName | Expected: select | Actual: " + tag);
		caseNo++;
		...
		sa.assertTrue(defaultVal.contains("SELECT"), "BUG: default not --SELECT-- | Expected: SELECT | Actual: " + defaultVal);
		caseNo++;
