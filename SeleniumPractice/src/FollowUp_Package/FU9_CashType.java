...
public class FU9_CashType extends FU2_Login {

	@Test
	public void validateCashType() throws Exception
	{
		System.out.println("========== CASH TYPE (Dropdown) ==========");
		int caseNo = 1;

		WebElement dd = driver.findElement(By.id("cashType"));
		Select sel = new Select(dd);

		boolean disp = dd.isDisplayed();
		System.out.println("Case " + caseNo + " | displayed | Expected: true | Actual: " + disp);
		sa.assertTrue(disp, "BUG: cashType not displayed | Expected: true | Actual: " + disp);
		caseNo++;

		boolean en = dd.isEnabled();
		System.out.println("Case " + caseNo + " | enabled | Expected: true | Actual: " + en);
		sa.assertTrue(en, "BUG: cashType not enabled | Expected: true | Actual: " + en);
		caseNo++;

		String tag = dd.getTagName();
		System.out.println("Case " + caseNo + " | tagName | Expected: select | Actual: " + tag);
		sa.assertEquals(tag, "select", "BUG: cashType tagName | Expected: select | Actual: " + tag);
		caseNo++;
		...
		sa.assertFalse(hasDup, "BUG: duplicate options | Expected: no duplicates | Actual: duplicates exist");
		caseNo++;
		...
		System.out.println("Case " + caseNo + " | select index 0 | Actual: " + idx0);
		caseNo++;
		...
		sel.selectByIndex(1);
		System.out.println("========== CASH TYPE Complete ==========\n");
	}
}
