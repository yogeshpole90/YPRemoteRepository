...
public class LD20_BankruptcyCaseDate extends LD2_Login {

	public void validateBankruptcyCaseDate() throws Exception {
		String bankVal = LD_ExcelUtil.getCellData(1, 15);
		if (!bankVal.equalsIgnoreCase("YES")) {
			logInfo("Bankruptcy Case Date", "Bankruptcy=" + bankVal, "SKIPPED - not YES"); return;
		}
		List<WebElement> list = driver.findElements(By.id("bankruptcyCaseDate"));
		if (list.isEmpty() || !list.get(0).isDisplayed() || !list.get(0).isEnabled()) {
			logInfo("Bankruptcy Case Date", "Field availability", "SKIPPED"); return;
		}
		WebElement field = list.get(0); String fn = "Bankruptcy Case Date";
		String excelVal = LD_ExcelUtil.getCellData(1, 16);
