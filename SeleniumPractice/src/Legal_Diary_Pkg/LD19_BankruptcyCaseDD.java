...
public class LD19_BankruptcyCaseDD extends LD2_Login {

	public void validateBankruptcyCaseDD() throws Exception {
		List<WebElement> list = driver.findElements(By.xpath("(//select[@id='bankruptcyCase'])"));
		if (list.isEmpty() || !list.get(0).isDisplayed()) {
			logInfo("Bankruptcy Case", "Field availability", "SKIPPED"); return;
		}
		WebElement field = list.get(0); String fn = "Bankruptcy Case";
		...
		log(fn, "Auto-fetched value", "Non-empty", selectedText, selectedText != null && !selectedText.equalsIgnoreCase("Select"));
