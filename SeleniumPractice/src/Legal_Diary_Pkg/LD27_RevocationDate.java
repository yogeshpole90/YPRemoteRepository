...
public class LD27_RevocationDate extends LD2_Login {

	public void validateRevocationDate() throws Exception {
		String revVal = LD_ExcelUtil.getCellData(1, 22);
		if (!revVal.equalsIgnoreCase("YES")) {
			logInfo("Revocation Date", "Revocation=" + revVal, "SKIPPED - not YES"); return;
		}
		List<WebElement> list = driver.findElements(By.id("revocationDate"));
		if (list.isEmpty() || !list.get(0).isDisplayed() || !list.get(0).isEnabled()) {
			logInfo("Revocation Date", "Field availability", "SKIPPED"); return;
		}
		WebElement field = list.get(0); String fn = "Revocation Date";
		String excelVal = LD_ExcelUtil.getCellData(1, 23);
