...
public class EmployeeMasterTest extends BaseTest {
    ...
    private void handleInput(WebElement f, String fieldName, String input, String expected, String desc,
            String checkType) throws Exception {
        // Date fields — use JS to set value
        if (fieldName.toLowerCase().contains("date") || fieldName.toLowerCase().contains("joining")) {
            jse.executeScript("arguments[0].value=''", f);
            if (!input.isEmpty() && !input.equalsIgnoreCase("Empty")) {
                jse.executeScript("arguments[0].value=arguments[1]", f, input);
            }
            Thread.sleep(300);
        } else {
